...
const SUBCLASS_DB = [
  ...
  {id:'racket-mastermind',       class_id:'rogue', subclass_type:'전문', name_ko:'지략가',      name_en:'Mastermind',
   summary:'지능에 의지하여 정교한 계략을 세우며, 다른 이가 3수 앞을 볼 때 10수 앞을 봅니다. 지식 회상으로 생물을 성공적으로 식별하면 다음 턴 시작까지 당신의 공격에 <strong>발묶임</strong>; 대성공이면 1분간.<br><strong>기술:</strong> 사회 + 주문학/자연학/오컬티즘/종교 중 1개. <strong>핵심 속성:</strong> 지능 선택 가능.'},
  {id:'racket-scoundrel',        class_id:'rogue', subclass_type:'전문', name_ko:'사기꾼',      name_en:'Scoundrel',
   summary:'재빠른 말솜씨, 아첨, 날카로운 혀로 위험을 피하고 곤란한 상황을 탈출합니다. 기만으로 성공적으로 속임(Feint) 시 다음 턴 종료까지 당신의 근접 공격에 <strong>발묶임</strong>; 대성공이면 모든 근접 공격에. 민첩/기교 근접 무기를 들고 속이면 즉시 <strong>자유 행동으로 한 걸음</strong>.<br><strong>기술:</strong> 기만, 외교. <strong>핵심 속성:</strong> 매력 선택 가능.'},
  {id:'racket-ruffian',          class_id:'rogue', subclass_type:'전문', name_ko:'건달',        name_en:'Ruffian',
   summary:'직접적 접근을 선호: 완력이나 위협. 어떤 무기로든 은밀 공격 피해를 줄 수 있습니다(단, d8 초과 단순 무기나 d6 초과 군용/고급 무기는 제외). 이 무기로 발묶임한 대상에 치명타 시 <strong>치명 특성 효과</strong> 적용.<br><strong>기술:</strong> 위협, 평갑 숙련. <strong>핵심 속성:</strong> 근력 선택 가능. 경갑 전문가/달인 획득 시 평갑에도 적용.'},
...
const TRAIT_DB = {
  ...
  '발묶임':'AC에 -2 상황 페널티. 협공, 넘어뜨려짐, 은신 공격 등으로 발생.',
  '마비':'완전히 움직일 수 없음. 발묶임. 순수 정신 행동만 가능.',
  '석화':'돌로 변함. 행동/감각 불가. AC 9, 경도 8, HP 50.',
  '엎드려짐':'바닥에 누움. 발묶임 + 공격에 -2. 일어나기에 1행동. 원거리 공격에 +1 AC.',
  '가속':'매 턴 추가 행동 1개(보통 용도 제한). 여러 가속은 누적 불가.',
  '억제':'이동 불가 + 발묶임 + 조작에 DC 5 단순 판정. 탈출로 빠져나옴.',
  ...
  '무의식':'의식 없음. AC -4, 감지 -4, 발묶임+눈멈. 행동 불가.',
...
const CONDITIONS_DATA = [
  ...
  {name:'서투름',en:'Clumsy',valued:true,max:4,desc:'민첩 기반 판정/DC에 상태 페널티 = 서투름 수치. AC(민첩), 반사 내성에도 적용.'},
  {name:'은폐',en:'Concealed',valued:false,desc:'잘 보이지 않음. 공격 시 DC 5 단순 판정 필요.'},
  ...
  {name:'현혹됨',en:'Dazzled',valued:false,desc:'시각 기반 대상이 은폐 취급.'},
  ...
  {name:'탈진',en:'Drained',valued:true,max:4,desc:'건강 기반 판정에 상태 페널티 = 탈진 수치. 최대 HP도 감소.'},
  ...
  {name:'발묶임',en:'Flat-Footed',valued:false,desc:'민첩 보너스가 AC에 적용되지 않음.'},
  ...
  {name:'고정',en:'Immobilized',valued:false,desc:'이동 속도 0. 회전은 가능.'},
  ...
  {name:'마비',en:'Paralyzed',valued:false,desc:'행동/반응 불가. AC에 -2 상황 페널티. 발묶임 상태.'},
  ...
  {name:'구속',en:'Restrained',valued:false,desc:'붙잡힘 + 고정 상태. 이동 속도 0.'},
  {name:'메스꺼움',en:'Sickened',valued:true,max:4,desc:'모든 판정/공격에 상태 페널티 = 메스꺼움 수치. 음식 섭취 불가.'},
  {name:'둔화',en:'Slowed',valued:true,max:3,desc:'매 턴 행동 수 = 3 - 둔화 수치.'},
  {name:'기절',en:'Stunned',valued:true,max:4,desc:'행동 수치가 기절 값만큼 소모됨. 매 턴 기절 수치만큼 행동 손실.'},
  {name:'현기증',en:'Stupefied',valued:true,max:4,desc:'지능/지혜/매력 기반 판정과 주문 DC에 상태 페널티 = 현기증 수치.'},
  ...
  {name:'과적',en:'Encumbered',valued:false,auto:true,desc:'부피가 5+근력 수정치 초과. 서투름 1, 이동 속도 10피트 감소. 부피 줄이면 자동 해제.'},
];
...
const ACTION_DB = [
  ...
  // ── 운동 (Athletics) 기술 행동 ──
  {id:'climb',         cat:'skill', cat_label:'운동 행동', name_ko:'등반',            name_en:'Climb',          cost:'1',        traits:['이동'],            req_skill:'athletics', req_rank:0, req_feat:null, summary:'<strong>특성:</strong> 이동<br><br>수직 표면을 오릅니다. 등반 성공 시 5피트 위로 이동(대성공 시 10피트, 실패 시 이동 안 됨, 대실패 시 추락). 등반 중에는 <strong>발묶임</strong>입니다. DC는 표면에 따라 다릅니다.'},
  ...
  {id:'shove',         cat:'skill', cat_label:'운동 행동', name_ko:'밀기',            name_en:'Shove',          cost:'1',        traits:['공격'],            req_skill:'athletics', req_rank:0, req_feat:null, summary:'<strong>특성:</strong> 공격<br>적을 뒤로 밉니다. 운동 판정을 적의 인내 DC에 대해 시도합니다. 대성공: 5피트 밀고 따라감 가능, 성공: 5피트 밀기, 실패: 효과 없음, 대실패: 발묶임.'},
  {id:'trip',          cat:'skill', cat_label:'운동 행동', name_ko:'넘어뜨리기',      name_en:'Trip',           cost:'1',        traits:['공격'],            req_skill:'athletics', req_rank:0, req_feat:null, summary:'<strong>특성:</strong> 공격<br>적을 엎드리게 합니다. 운동 판정을 적의 반사 DC에 대해 시도합니다. 대성공: 대상이 엎드리고 1d6 둔기 피해, 성공: 엎드림, 실패: 효과 없음, 대실패: 당신이 발묶임.'},
  {id:'grapple',       cat:'skill', cat_label:'운동 행동', name_ko:'붙잡기',          name_en:'Grapple',        cost:'1',        traits:['공격'],            req_skill:'athletics', req_rank:0, req_feat:null, summary:'<strong>특성:</strong> 공격<br>빈 손으로 적을 붙잡으려 합니다. 운동 판정을 적의 인내 DC에 대해 시도합니다. 대성공: 속박(restrained), 성공: 붙잡힘(grabbed), 실패: 효과 없음, 대실패: 적이 당신을 붙잡거나 당신이 발묶임.'},
  {id:'disarm',        cat:'skill', cat_label:'운동 행동', name_ko:'무장해제',        name_en:'Disarm',         cost:'1',        traits:['공격','교묘'],     req_skill:'athletics', req_rank:2, req_feat:null, summary:'<strong>특성:</strong> 공격<br>적의 무기나 물건을 빼앗으려 합니다. 운동 판정을 적의 반사 DC에 대해 시도합니다. 대성공: 물건이 땅에 떨어짐, 성공: 대상이 -2 상황 페널티(공격과 조작), 실패: 효과 없음, 대실패: 발묶임.'},
  ...
  {id:'reposition',    cat:'skill', cat_label:'운동 행동', name_ko:'위치 변경',       name_en:'Reposition',     cost:'1',        traits:['공격'],            req_skill:'athletics', req_rank:0, req_feat:null, summary:'<strong>특성:</strong> 공격<br>적을 원하는 위치로 강제 이동시킵니다. 운동 판정을 적의 인내 DC에 대해 시도합니다. 대성공: 5피트 이동시키고 당신도 따라감, 성공: 5피트 이동, 실패: 효과 없음, 대실패: 발묶임.'},

  // ── 곡예 (Acrobatics) 기술 행동 ──
  {id:'balance',       cat:'skill', cat_label:'곡예 행동', name_ko:'균형 잡기',       name_en:'Balance',        cost:'1',        traits:['이동'],            req_skill:'acrobatics', req_rank:0, req_feat:null, summary:'<strong>특성:</strong> 이동<br><br>좁거나 불안정한 표면 위를 이동합니다. 곡예 판정을 시도합니다. DC는 표면에 따라 달라집니다. 균형 잡기를 시도하는 동안 당신은 <strong>발묶임(off-guard)</strong>입니다.<br><strong>대성공:</strong> 이동 속도까지 이동하며 지형을 험지로 취급합니다(일반적으로 절반 속도).<br>'},
  ...
  {id:'feint',         cat:'skill', cat_label:'기만 행동', name_ko:'페인트',          name_en:'Feint',          cost:'1',        traits:[],                  req_skill:'deception', req_rank:2, req_feat:null, summary:'<strong>특성:</strong> 정신<br><br>기만적인 동작으로 적을 진짜 공격에 대비시키지 못하게 합니다. 기만 판정을 대상의 감지 DC에 대해 시도합니다.<br><strong>대성공:</strong> 대상이 다음 턴 종료까지 당신의 근접 공격에 발묶임.<br>'},
  ...
  {id:'sneak-attack-ability',cat:'feat',cat_label:'재주 행동',name_ko:'기습 공격',    name_en:'Sneak Attack',   cost:'passive',  traits:[],                  req_skill:null, req_rank:0, req_feat:'기습 공격', summary:'발묶임(off-guard) 상태인 생물에 민첩/기교 근접 무기, 민첩/기교 비무장 공격, 원거리 무기/비무장 공격으로 타격 시 <strong>추가 1d6 정밀 피해</strong>. 투척 근접 무기는 민첩/기교여야 합니다. 5, 11, 17레벨에 피해 주사위가 1개씩 증가.'},
